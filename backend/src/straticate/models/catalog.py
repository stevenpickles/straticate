...
Architecture-specific manifest fields (``default_inference_parameters``,
``artifact``, ``licensing``) are absent from :class:`~straticate.schemas.Model`
and are therefore dropped on load: they can never reach the API. Users choose
modes and quality tiers, never inference parameters.
"""
...
from straticate.schemas import Model, QualityOption, QualityTier, SeparationMode
...
class _CatalogFile(BaseModel):
    ...
    Entry fields outside :class:`~straticate.schemas.Model` (``schema_version``,
    ``artifact``, ``licensing``, ``default_inference_parameters``, …) are
    accepted and ignored here; they belong to the model manager and the
    inference package, not to the API surface.
    """
...
class ModelCatalog:
    ...
    Args:
        models: The catalog's models, in presentation order.
    ...
    Raises:
        ModelCatalogError: On a duplicate model ID, models of one mode
            disagreeing on stems, or two models of one mode claiming the same
            quality tier.
    """
        ...
        *,
        ...
        for model in self._models:
            if model.id in self._by_id:
                raise ModelCatalogError(f"{source}: duplicate model ID {model.id!r}.")
