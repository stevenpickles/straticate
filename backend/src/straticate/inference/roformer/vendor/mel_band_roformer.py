...
complete list of Straticate's modifications. Do not "clean this up": the layer
shapes here are what a published checkpoint loads into, so this file is pinned,
not maintained.
...
Reads: .attend.Attend, .mel_filters.mel_filter_bank, rotary_embedding_torch.RotaryEmbedding, beartype, einops, torch
"""
...
# Straticate modification: librosa's mel filter bank, transcribed into
# ``mel_filters.py`` so this package does not depend on librosa (and its numba /
# llvmlite / scipy chain) for one pure function. Verified bit-identical.
from .mel_filters import mel_filter_bank as compute_mel_filter_bank
...
class MelBandRoformer(Module):
    ...
    def __init__(
        ...
        # Straticate modification: an explicit rectangular window. This probe
        # only reads ``.shape[1]`` (the bin count, which no window can change),
        # but torch warns when ``window`` is omitted and the backend suite
        # treats a warning as a finding -- so we pass the window torch's own
        # warning text recommends for exactly this case.
        ...
        mel_filter_bank_numpy = compute_mel_filter_bank(sample_rate=sample_rate, n_fft=stft_n_fft, n_mels=num_bands)
